...
class AppConfig:
    ...
    # Cross-camera Re-ID (OSNet x1.0 + Market-1501 weights recommended)
    REID_SIMILARITY_THRESHOLD = 0.67
    REID_GALLERY_POLLUTION_SIM = 0.50
    REID_MAX_GALLERY_SIZE = 16
    REID_STALE_TIMEOUT_SEC = 300.0
    REID_REFRESH_INTERVAL_FRAMES = 16
    ...
    REID_WEIGHTS_FILENAME = "osnet_x1_0_market1501.pth"
    REID_WEIGHTS_PATH = os.getenv("REID_WEIGHTS_PATH", "").strip()
    REID_AUTO_DOWNLOAD_WEIGHTS = os.getenv("REID_AUTO_DOWNLOAD", "true").lower() == "true"
    REID_GDRIVE_MARKET1501_FILE_ID = "1vduhq5DpN2q1g4fYEZfPI17MJeh9qyrA"
    REID_TOP_K_GALLERY_SIM = 3
    REID_GENDER_GATE_ENABLED = True
    REID_ADAPTIVE_THRESHOLD_SMALL_GALLERY_ADD = 0.05
    REID_ADAPTIVE_THRESHOLD_LARGE_GALLERY_SUB = 0.03
    REID_LARGE_GALLERY_MIN_COUNT = 5
    REID_AMBIGUOUS_MATCH_MARGIN = 0.03
    REID_VIEWPOINT_DIVERSITY_MIN_SIM = 0.70
    REID_EMA_ALPHA = 0.10
    REID_TRANSITION_PENALTY = 0.15
    REID_ADJACENT_BONUS = 0.05
    REID_SAME_CAM_REENTRY_SEC = 120.0
    REID_SAME_CAM_THRESHOLD_RELAX = 0.08
    # "from_cam|to_cam": (min_seconds, max_seconds) — penalize match if transition time out of range
    REID_CAMERA_TRANSITIONS = {}
    # List of (from_cam, to_cam) pairs that get REID_ADJACENT_BONUS on similarity
    REID_ADJACENT_CAMERA_PAIRS = []
